...
namespace RevitSketchPoC.Chat.Services
{
    ...
    public sealed class LlmChatService
    {
        private const string SystemPromptBase =
            ...
            "If the JSON includes \"footprintRepairHints\" (floors/ceilings with suggestedWallIds), prefer those wallIds for analyze_floor_wall_footprint / repair_floor_to_wall_footprint and the ceiling equivalents when the user works on those slabs — especially on busy levels or when planGeometryInActiveView is omitted in 3D. " +
            ...
            "- analyze_floor_wall_footprint: read-only check — floorId or floorIds (from context); optional wallIds (otherwise walls near the slab from heuristics — on busy levels pass wallIds from context footprintRepairHints.floors for that floorId); optional toleranceMeters (default 0.08), areaRatioTolerance (default 0.06). Writes short log lines; optional includeJson:true for full JSON.\n" +
            "- repair_floor_to_wall_footprint: fixes slabs from chained wall location curves. Requires floorId; strongly recommended wallIds on levels with many walls (use footprintRepairHints). alignTo: wall_centerline (default), wall_inside, or wall_outside. Uses model geometry — works in 3D view; planGeometryInActiveView omitted is NOT a blocker.\n" +
            ...
            "- analyze_ceiling_wall_footprint: read-only check for ceilings — ceilingId or ceilingIds; optional wallIds, toleranceMeters (default 0.08), areaRatioTolerance (default 0.06). Logs ceiling-vs-wall-chain footprint metrics and suggestedRepair when mismatch.\n" +
            "- repair_ceiling_to_wall_footprint: deletes/recreates a ceiling from wall location curves on the same level. Requires ceilingId; optional wallIds; alignTo: wall_centerline (default), wall_inside, or wall_outside. Copies ceiling type, offset, and comments when possible.\n" +
