...
namespace RevitSketchPoC.Chat.Services
{
    ...
    public sealed class LlmChatService
    {
        private const string SystemPromptBase =
            ...
            "If the JSON includes \"namedTypesForRevitOps\", use those exact strings for wallTypeName, floorTypeName, ceilingTypeName, doorTypeName, windowTypeName, structuralColumnTypeNames or architecturalColumnTypeNames (pillarTypeName / columnTypeName for create_pillar), structuralFramingTypeNames (beamTypeName for create_beam), railingTypeNames (railingTypeName / stairRailingTypeName / guardrailTypeName for create_guardrail, create_railing, or **create_stairs** stair-hosted railings), familyTypeName, stairsTypeName (create_stairs), and sampleLoadableFamilyTypes when emitting revitOps. " +
            ...
            "  **Ceiling height (important):** default **`ceilingKind`** is **`false_ceiling`** (teto falso): the ceiling sits **below** the wall top by **`falseCeilingDropMeters`** (default ~0.30 m from plugin), but **never** below the top of doors, windows, or wall openings (uses insert bounding boxes). Use **`ceilingKind` `slab_painted`** (aliases **`laje`**, **`painted_slab`**) for a ceiling **flush** with wall tops (laje pintada), still clamped above openings if needed. Half-room false + half painted = **two** `create_ceiling_*` ops with different boundaries and kinds, plus a partition wall between — the plugin does not merge zones automatically.\n\n" +
            ...
            "- create_floor_from_room: **PRIMARY for new floors when a Room exists** — roomId (or elementId when the element is a Room). Uses the room's computed boundary (arcs/circles OK). Optional floorTypeName, name (comment), boundaryLocation: **finish (default)** = inside room up to wall finish faces; use **center** only if you want the slab on the wall axis/centreline; also coreBoundary, coreCenter.\n" +
            ...
            "- create_ceiling_from_room: **PRIMARY for new ceilings when a Room exists** — roomId (or elementId for Room); optional ceilingTypeName, name, boundaryLocation (default finish; same options as floor). **ceilingKind** (optional): **`false_ceiling`** (default, teto falso) or **`slab_painted`** / **`laje`** (colado ao topo da parede). **falseCeilingDropMeters** (optional): vertical drop for false ceiling (metres; default from plugin ~0.30). Height is always clamped **above** doors, windows, and wall openings.\n" +
            "- create_ceiling: **Fallback** — same geometry options as create_floor (boundary, boundarySegments, or circle), ceilingTypeName; same **ceilingKind** / **falseCeilingDropMeters** as create_ceiling_from_room.\n" +
            ...
            "- change_ceiling_kind: **fix or switch ceiling vertical mode** — ceilingId (or elementId); **ceilingKind** `false_ceiling` or `slab_painted` (aliases: teto_falso, laje, …); optional **falseCeilingDropMeters**. Recomputes Height Offset From Level from nearby walls and openings.\n" +
            ...
            "- create_beam: structural framing (viga) — **beamTypeName** must match **structuralFramingTypeNames** in namedTypesForRevitOps (or \"Family : Type\"); axis in plan: **startX/startY/endX/endY** or **start**/**end** `{x,y}` in metres (same convention as create_wall); **levelName** (reference level); optional **zOffsetMeters** (offset of the beam line from the level elevation); optional **name** (Comments).\n" +
            "- create_guardrail (alias **create_railing**): guarda-corpo / corrimão Revit — **railingTypeName** (or **guardrailTypeName**) must match **railingTypeNames** in namedTypesForRevitOps (type name string). Path in plan: **startX/startY/endX/endY** or **start**/**end** `{x,y}` in metres; **levelName**; optional **zOffsetMeters**. Baluster spacing, rail profile, and height are defined by the **RailingType** in the project (not overridden by this JSON op); optional **name** (Comments).\n" +
            ...
            "- create_stairs: straight component stair between two levels — bottomLevelName (or baseLevelName), topLevelName (must be higher), startX/startY and endX/endY in metres (plan path on the bottom level); optional stairsTypeName (match namedTypesForRevitOps.stairsTypeNames); optional justification: left | center | right (run vs path). **Automatic stair railings are removed by default**; set **keepStairsRailings** true only if the user explicitly wants to keep Revit's auto railings from the stair type. **Stair railing type:** optional **railingTypeName** / **stairRailingTypeName** / **guardrailTypeName** (same strings as **railingTypeNames** in context) — if **keepStairsRailings** is true, retipes existing auto railings to that type; if false (default), removes railings then creates them with **Railing.Create** for that type. Optional **stairRailingPlacement** / **railingPlacementPosition**: **treads** (default) or **stringer**. For freeform paths not tied to the stair host, use **create_guardrail** in separate ops.\n" +
            ...
            "All revitOps in the JSON array are applied in one Revit transaction except create_wall_roman_arch_profile, create_wall_custom_profile_void, and create_stairs (they commit earlier ops first). Very large batches can be slow or fail mid-run — prefer reasonable sizes or the Sketch-to-BIM flow for full floor plans.\n" +
