...
namespace RevitSketchPoC.Chat.Services
{
    ...
    public sealed class LlmChatService
    {
        private const string SystemPromptBase =
            ...
            "- create_ramp: **same fields as create_stairs** (bottom + top level and path in metres). The Revit API used here creates a **straight stair run** (not a native OST_Ramps element). Optional rampTypeName: if it matches a **stairs type** name, that type is used; otherwise the first stair type whose name contains \"ramp\" or \"rampa\" (else default stair type). Prefer **stairsTypeName** when you need an exact type. For loadable ramp families use create_family_instance.\n" +
            ...
            "All revitOps in the JSON array are applied in one Revit transaction except create_wall_roman_arch_profile, create_wall_custom_profile_void, create_stairs, and create_ramp (they commit earlier ops first). Very large batches can be slow or fail mid-run — prefer reasonable sizes or the Sketch-to-BIM flow for full floor plans.\n" +
